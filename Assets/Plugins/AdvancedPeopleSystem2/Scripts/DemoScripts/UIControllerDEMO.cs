...
namespace AdvancedPeopleSystem
{
    ...
    public class UIControllerDEMO : MonoBehaviour
    {
        ...
        public CustomizableCharacter customizableCharacter;
        [Space(15)]
        ...
        private void Start()
        {
            if (customizableCharacter.Settings.name == "FemaleSettings")
            {
        ...
        {
            customizableCharacter.SwitchCharacterSettings(name);
            if (name == "Male")
            {
                maleUI.gameObject.SetActive(true);
                femaleUI.gameObject.SetActive(false);
        ...
        {
            var faceBlendshape = customizableCharacter.GetBlendshapeDatasByGroup(CharacterBlendShapeGroup.Face);


            customizableCharacter.SetBlendshapeValue(faceBlendshape[index].type, faceShapeSliders[index].value);
        }

        public void SetHeadOffset()
        {
            customizableCharacter.SetBlendshapeValue(CharacterBlendShapeType.Head_Offset, headOffsetSlider.value);
        }

        public void BodyFat()
        {
            customizableCharacter.SetBlendshapeValue(CharacterBlendShapeType.Fat, fatSlider.value);
        }
        public void BodyMuscles()
        {
            customizableCharacter.SetBlendshapeValue(CharacterBlendShapeType.Muscles, musclesSlider.value);
        }
        public void BodyThin()
        {
            customizableCharacter.SetBlendshapeValue(CharacterBlendShapeType.Thin, thinSlider.value);
        }

        public void BodySlimness()
        {
            customizableCharacter.SetBlendshapeValue(CharacterBlendShapeType.Slimness, slimnessSlider.value);
        }
        public void BodyBreast()
        {
            customizableCharacter.SetBlendshapeValue(CharacterBlendShapeType.BreastSize, breastSlider.value);
        }
        public void SetHeight()
        {
            customizableCharacter.SetHeight(heightSlider.value);
        }
        public void SetHeadSize()
        {
            customizableCharacter.SetHeadSize(headSizeSlider.value);
        }
        int lodIndex;
        ...
        public void SavesPanel_Select(bool v)
        {
            ...
            {
                var saves = customizableCharacter.GetSavedCharacterDatas();
        ...
        {
            var saves = customizableCharacter.GetSavedCharacterDatas();
            customizableCharacter.ApplySavedCharacterData(saves[index]);
            UpdateUI();
        }
        public void EmotionsChange_Event(int index)
        {
            var anim = customizableCharacter.Settings.characterAnimationPresets[index];
            if (anim != null)
                customizableCharacter.PlayBlendshapeAnimation(anim.name, 2f);
        }
        public void HairChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Hair, index);
        }
        public void BeardChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Beard, index);
        }
        public void ShirtChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Shirt, index);
        }
        public void PantsChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Pants, index);
        }
        public void ShoesChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Shoes, index);
        }
        public void BackpackChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Item1, index);
        }
        public void HatChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Hat, index);
        }

        public void AccessoryChange_Event(int index)
        {
            customizableCharacter.SetElementByIndex(CharacterElementType.Accessory, index);
        }
        ...
        Dictionary<CharacterElementType, Image> selectedButtons = new Dictionary<CharacterElementType, Image>()
    {
        ...
        {
            int index = customizableCharacter.characterSelectedElements.GetSelectedIndex(type);
        ...
        public void UpdateUI()
        {
            if (customizableCharacter != null)
            {
                foreach (var type in Enum.GetValues(typeof(CharacterElementType)))
                {
                    ...
                    SkinColorButtonColor.color = customizableCharacter.GetBodyColor(BodyColorPart.Skin);
                    EyeColorButtonColor.color = customizableCharacter.GetBodyColor(BodyColorPart.Eye);
                    HairColorButtonColor.color = customizableCharacter.GetBodyColor(BodyColorPart.Hair);
                    UnderpantsColorButtonColor.color = customizableCharacter.GetBodyColor(BodyColorPart.Underpants);
                }
                headSizeSlider.value = customizableCharacter.headSizeValue;
                headOffsetSlider.value = customizableCharacter.GetBlendshapeData(CharacterBlendShapeType.Head_Offset)?.value ?? 0;
                heightSlider.value = customizableCharacter.heightValue;
                if (breastSlider != null)
                    breastSlider.value = customizableCharacter.GetBlendshapeData(CharacterBlendShapeType.BreastSize)?.value ?? 0;

                if (fatSlider != null)
                    fatSlider.value = customizableCharacter.GetBlendshapeData(CharacterBlendShapeType.Fat)?.value ?? 0;

                if (musclesSlider != null)
                    musclesSlider.value = customizableCharacter.GetBlendshapeData(CharacterBlendShapeType.Muscles)?.value ?? 0;

                if (thinSlider != null)
                    thinSlider.value = customizableCharacter.GetBlendshapeData(CharacterBlendShapeType.Thin)?.value ?? 0;

                if (slimnessSlider != null)
                    slimnessSlider.value = customizableCharacter.GetBlendshapeData(CharacterBlendShapeType.Slimness)?.value ?? 0;
                ...
                {
                    var blendshapeData = customizableCharacter.GetBlendshapeData(facialSlider.transform.parent.name);
        ...
        {
            customizableCharacter.SaveCharacterToFile(CharacterCustomizationSetup.CharacterFileSaveFormat.Json);
        }
        public void ClearFromFile()
        {
            SavesPanel.gameObject.SetActive(false);
            customizableCharacter.ClearSavedData();
        }
        ...
        public void PlayAnim()
        {
            ...
            customizableCharacter.GetAnimator().SetBool("walk", walk_active);
