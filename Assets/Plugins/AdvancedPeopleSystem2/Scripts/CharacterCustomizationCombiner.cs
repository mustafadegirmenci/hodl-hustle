...
namespace AdvancedPeopleSystem
{
    public class CharacterCustomizationCombiner
    {
        ...
        static CustomizableCharacter _currentCustomizableCharacter;
        ...
        public static void AddBakeToQueue(CustomizableCharacter customizableCharacter, bool bakeBlendshapes = false, GameObject exportInCustomObject = null, float blendshapeAddDelay = 0.001f, Action<List<SkinnedMeshRenderer>> callback = null)
        {
            if (customizableCharacter.IsBaked())
                customizableCharacter.ClearBake();
            ...
            newBakeQuest.CustomizableCharacter = customizableCharacter;
        ...
        public static void EnqueueCharacterBake()
        {
            ...
            {            
                ...
                {                  
                    MakeCombinedMeshes(enqueueCharacter.CustomizableCharacter, enqueueCharacter.bakeBlendshapes, enqueueCharacter.exportInCustomObject, enqueueCharacter.blendshapeAddDelay, enqueueCharacter.callback);                 
                }
            }
        }

        public static List<SkinnedMeshRenderer> MakeCombinedMeshes(CustomizableCharacter customizableCharacter, bool bakeBlendshapes = false, GameObject exportInCustomObject = null, float blendshapeAddDelay = 0.001f, Action<List<SkinnedMeshRenderer>> callback = null)
        {
            ...
            customizableCharacter.CurrentCombinerState = CombinerState.InProgressCombineMesh;

            _currentCustomizableCharacter = customizableCharacter;


            SkinnedMeshRenderer bodyMesh = customizableCharacter.GetCharacterPart("Head").skinnedMesh[0];
            ...
            for (int i = 0; i < (customizableCharacter.MaxLODLevels - customizableCharacter.MinLODLevels) + 1; i++)
            {
                LODMeshInstances.Add(new MeshInstance());
                foreach(var m in customizableCharacter.GetAllMeshesByLod(i))
                {
                    SelectMeshes(m, i);
                }
            }
            ...
            SkinnedMeshRenderer meshForCombine = customizableCharacter.GetCharacterPart("Combined").skinnedMesh[0];
            List<SkinnedMeshRenderer> newObjectsForCombine = customizableCharacter.GetCharacterPart("Combined").skinnedMesh;
            ...
            for (int i = 0; i < LODMeshInstances.Count; i++)
            {
                ...
                if (!useExportToAnotherObject)
                {
                    if (customizableCharacter.CurrentCombinerState != CombinerState.InProgressBlendshapeTransfer)
                        customizableCharacter.CurrentCombinerState = CombinerState.InProgressBlendshapeTransfer;
                }
                if(!bakeBlendshapes)
                    customizableCharacter.StartCoroutine(BlendshapeTransfer(meshInstance, blendshapeAddDelay, newObjectsForCombine[i], i, exportInCustomObject == null));
            }
            ...
            {
                if (!useExportToAnotherObject)
                    customizableCharacter.CurrentCombinerState = CombinerState.Combined;
        ...
        static IEnumerator BlendshapeTransfer(MeshInstance meshInstance, float waitTime, SkinnedMeshRenderer smr, int lod, bool yieldUse = true)
        {
            ...
            CustomizableCharacter customizableCharacterSystem = _currentCustomizableCharacter;
            ...
            if (lod == (customizableCharacterSystem.MaxLODLevels - customizableCharacterSystem.MinLODLevels))
            {
                if (!useExportToAnotherObject)
                    customizableCharacterSystem.CurrentCombinerState = CombinerState.Combined;
                _callback?.Invoke(returnSkinnedMeshes);
